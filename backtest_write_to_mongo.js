...
client.connect(function(err, client) {
	...
	var cursor = db.collection('binanceOrderBook').find({}).forEach(function(doc) {
		arbitrageFunctions.findTriangularArbitrage(doc.timestamp,doc.binanceOrderData,checkOrderQuantity=false, profitThresholdPercent=0.0, baseCurrency='USDT', debug=false, feePercent=0.0, callback=function(response) {
		db.collection('binanceBackTestResults').insertOne(response, function(){});
	});
	}, function(err) {
		console.log(err);
		client.close();
